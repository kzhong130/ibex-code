...
string PIRServer::serialize_reply(PirReply &reply) {
    stringstream stream;
    int len = reply.size();
    string res = string((const char *)&len, sizeof(int));
    int output_size = 0;
    for (int i = 0; i < reply.size(); i++){
        evaluator_->mod_switch_to_inplace(reply[i], context_->last_parms_id());
        output_size += reply[i].save(stream);
        // string cipher_str = stream.str();
        // int ciphert_len = cipher_str.size();
        // res.append(string((const char *)&ciphert_len, sizeof(int)));
        // res.append(cipher_str);
        // cout << "finish " << i << endl;
    }
